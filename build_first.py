def main(min_transf=-105, cv_buildingid=False, cv_floor=False, cv_lat_lon=False, knn=True):
    ...
    distance_75_xgb = np.percentile(
        np.sqrt((lat_lon_preds["LONGITUDE"] - validation["LONGITUDE"])**2
                + (lat_lon_preds["LATITUDE"] - validation["LATITUDE"])**2)
        + 4*abs(x_test["FLOOR"] - validation["FLOOR"]) + 50*(x_test["BUILDINGID"] != validation["BUILDINGID"]),
        75
    )

    print(accuracy_score(validation["BUILDINGID"], x_test["BUILDINGID"]))
    print(accuracy_score(validation["FLOOR"], x_test["FLOOR"]))
    print(mean_absolute_error(validation["LATITUDE"], lat_lon_preds["LATITUDE"]))
    print(mean_absolute_error(validation["LONGITUDE"], lat_lon_preds["LONGITUDE"]))
    print(distance_75_xgb)
